...
stock_data = pd.read_pickle("usa_131_per_size_ranks_False.pkl")#mettici dataset_path

stock_data = stock_data[stock_data["size_grp"] == "micro"]
#stock_data = stock_data[stock_data["id"]%6 == 0]

# benchmark_path = f"/home/{os.environ['USER']}/SandP benchmark.csv" #QUESTO

SP_benchmark = pd.read_csv("SandP benchmark.csv")#mettici benchmark_path
...
def produce_random_feature_managed_returns(P, stock_data, signals, scale=1, num_seeds=2):
  """
...
stock_data = produce_random_feature_managed_returns(P=250, stock_data=stock_data, signals=stock_data, scale=1, num_seeds=2)
...
dates_to_save = []
first_t = 61
last_T =  61#len(months_list)-2 #first_t + 10
for t in range(first_t, last_T):
    ...
    print(f'RITORNI: {predicted}')#commenta questo

    # model_path = os.path.join(output_dir, f"model_weights_t{t}.pt")
    # torch.save(model.state_dict(), model_path) QUESTI
    ...
    #print(predicted, R_t_plus_one.mean().item() ) questo no
...
#csv_path = os.path.join(output_dir, "lele_DF.csv") questo
